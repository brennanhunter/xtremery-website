...
const gameProjects = [
  {
    ...
    insights: [
      ...
      {
        title: '🧠 Puzzle Design',
        description: 'Puzzles are intentionally varied and often fuse elemental powers with movement. You might freeze platforms, burn doors, or trigger switches with earthquake pulses. Often, puzzles combine order-based sequencing with environmental interaction and mobility challenges.',
        takeaway: 'A great puzzle forces you to think about timing, positioning, and power synergy — not just logic.'
      },
      {
        title: '🧭 Level Flow',
        description: 'Levels are structured to teach through doing. The player is first introduced to the level’s core mechanics or ideas in a safe, simple context. From there, each segment builds complexity — adding more layers of timing, enemy placement, or combo potential.',
        takeaway: 'Every level is a lesson. First, we teach; then we test.'
      },
      {
        title: '🌟 Progression Philosophy',
        description: 'The sprite acts as a narrative and mechanical tether — granting new abilities and giving just enough guidance to push players into exploration. Rather than spoon-feeding tutorials, the game prefers environmental hints and letting players discover the full depth through experimentation.',
  ...
  },
  {
    ...
    insights: [
      {
        title: '🧩 Puzzle Design',
        description: 'Puzzles mix logic and timing, where the challenge lies in knowing when to activate and how to move. The single-button constraint amplifies the decision-making pressure.',
        takeaway: 'Timing becomes a decision, not just a skill.'
      },
      {
        title: '👁️ Teaching Without Words',
        description: 'A pillar in an empty room invites experimentation. Like Mario, Shadow Mind teaches through interaction, not instruction.',
        takeaway: 'Discovery is more powerful than instruction.'
      },
      {
        title: '🔁 Simplicity with Depth',
        description: 'To keep things interesting under a one-button rule, environmental mechanics like anti-gravity and ice sliding were created to push the challenge further.',
        takeaway: 'Limitation breeds creativity — and challenge.'
      },
      {
        title: '🧠 The “Aha!” Philosophy',
        description: 'Good puzzles in Shadow Mind should feel like you finally noticed what was always in front of you. The solutions must feel fair and self-earned.',
    ...
    insights: [
      {
        ...
        description: 'Working under a 24-hour deadline meant cutting corners while still aiming for clarity. Every system had to be fast, readable, and breakproof.',
        takeaway: 'Speed forces clarity. You either make it work, or cut it.'
      },
      {
        title: '🎯 Chasing a Dream (Literally)',
        description: 'We interpreted the theme as escaping a nightmare — the dream is something you earn by collecting, surviving, and unlocking the final gate.',
        takeaway: 'Themes work best when they’re embodied, not just referenced.'
      },
      {
        title: '🛠 Leading as the Backbone',
        description: 'With most teammates new to development, I took the lead on design, Blueprints, and team workflow. It taught me how to ship fast while carrying others.',
...
export default function GameDesignSection() {
  ...
  return (
    <section id="game-design" className="scroll-mt-32 text-center max-w-6xl mx-auto py-24 px-6 space-y-20">
      {/* Intro and Game */}
      <motion.div
        variants={fadeUp}
        initial="hidden"
          ...
          >
            <div className="relative w-full h-full transition-transform duration-700 transform-style-preserve-3d group-hover:rotate-y-180">
              ...
              <div className="absolute inset-0 h-full bg-white p-6 rounded-xl shadow-xl backface-hidden flex flex-col justify-center">
                <h3 className="text-2xl font-bold text-purple-700 mb-2">{game.name}</h3>
              ...
              {/* Back Face */}
              <div className="absolute inset-0 h-full bg-gradient-to-br from-purple-100 to-blue-100 p-6 rounded-xl shadow-xl rotate-y-180 backface-hidden flex flex-col justify-between">
                <div>
                ...
                <button
                  ...
                  className="mt-4 bg-purple-600 text-white py-2 px-4 rounded-lg text-sm hover:bg-purple-700 transition"
                >
                  More Info
                </button>
              </div>
      ...
      <AnimatePresence>
        {openGame && (
          ...
          >
            ...
            >
              <button
                className="absolute top-4 right-4 text-gray-500 hover:text-purple-700 font-bold text-xl"
                onClick={() => setOpenGame(null)}
              >
                ×
              </button>
              <h3 className="text-3xl font-extrabold text-purple-700 mb-4">{openGame.name}</h3>
              ...
              {openGame.trailer ? (
                <div className="mb-6">
                  <h4 className="text-lg font-bold text-purple-800 mb-2">Watch the Trailer</h4>
              ...
              {openGame.insights?.length > 0 && (
                <div className="space-y-6">
                  {openGame.insights.map((section, idx) => (
                    <div key={idx}>
                      <h4 className="text-lg font-bold text-purple-800 mb-1">{section.title}</h4>
                      <p className="text-sm text-gray-700 mb-1">{section.description}</p>
                      <p className="text-sm italic text-purple-600">{section.takeaway}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {openGame.itchLink && (
                <div className="flex justify-center mt-8">
                  <a
                    ...
                    className="bg-gradient-to-r from-purple-600 to-blue-600 text-white font-semibold px-6 py-3 rounded-lg shadow-md hover:scale-105 transition-transform duration-300"
                  >
                    🚀 Play on itch.io
                  </a>
                </div>
